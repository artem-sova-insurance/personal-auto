...
function DriverCard({ index, driver, onChange, onRemove, t }) {
  ...
  const handlePhotoChange = (e) => {
    ...
    if (!file) return;
    upd('licensePhotoName', file.name);
    const reader = new FileReader();
    reader.onload = (ev) => upd('licensePhotoData', ev.target.result);
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 mb-4 bg-gray-50 relative animate-slide-down">
      ...
      <div className="grid grid-cols-2 gap-3">
        <FormField id={`dfn-${index}`} type="text" label={t('drivers.firstName')} value={driver.firstName} onChange={(v) => upd('firstName', v)} placeholder="Jane" required />
        <FormField id={`dln-${index}`} type="text" label={t('drivers.lastName')}  value={driver.lastName}  onChange={(v) => upd('lastName', v)}  placeholder="Smith" required />
      </div>

      <FormField id={`ddob-${index}`} type="date"   label={t('drivers.dateOfBirth')}    value={driver.dateOfBirth}  onChange={(v) => upd('dateOfBirth', v)} required />
      <FormField id={`drel-${index}`} type="select" label={t('drivers.relationship')}   value={driver.relationship} onChange={(v) => upd('relationship', v)} options={relOptions} required />

      {/* License info */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
        <FormField id={`dlnum-${index}`} type="text"   label={t('drivers.licenseNumber')} value={driver.licenseNumber} onChange={(v) => upd('licenseNumber', v)} placeholder="D123-456-78-900-0" required />
        <FormField id={`dlst-${index}`}  type="select" label={t('drivers.licenseState')}  value={driver.licenseState}  onChange={(v) => upd('licenseState', v)}  options={STATE_OPTIONS} required />
      </div>
...
export default function DriversStep({ t, data, update, onNext, onBack }) {
  ...
  const updateDriver = (i, d) => { const n = [...drivers]; n[i] = d; update('additionalDrivers', n); };
  ...
  return (
    <div className="animate-fade-in">
      ...
      {data.isOnlyDriver === 'no' && (
        <>
          {drivers.map((d, i) => (
            <DriverCard key={i} index={i} driver={d} onChange={(v) => updateDriver(i, v)} onRemove={() => removeDriver(i)} t={t} />
          ))}
          <button onClick={addDriver} className="w-full py-2.5 rounded-xl border-2 border-dashed border-brand-300 text-brand-700 font-semibold text-sm hover:bg-brand-50 transition-colors mb-4">
            {t('drivers.addDriver')}
          </button>
        </>
      )}
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <button onClick={onNext} disabled={!data.isOnlyDriver} className="px-6 py-2.5 rounded-xl bg-brand-600 text-white font-semibold text-sm hover:bg-brand-700 transition-colors shadow-sm disabled:opacity-40 disabled:cursor-not-allowed">
          {t('nav.next')} →
        </button>
      </div>
    </div>
